...
async function pagination({ model, query = {}, options, size, page, populates = {}, dis }) {
  if (typeof query === 'string') {
    query = qs.parse(query)
  }
  let result = await mongoPage(model).find(query).sort({"_id": -1}).populate(populates).select(options).size(size).page(page).display(dis).exec()
